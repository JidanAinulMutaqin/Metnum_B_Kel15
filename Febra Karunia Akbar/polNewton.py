def divided_difference(x_values, y_values):
    ...
    for i in range(n):
        table[i][0] = y_values[i]  # Inisialisasi tabel dengan nilai y

    for j in range(1, n):
        for i in range(n - j):
            table[i][j] = (table[i + 1][j - 1] - table[i][j - 1]) / (x_values[i + j] - x_values[i])  # Menghitung perbedaan terbagi

    return table

def newton_interpolation(x, x_values, table):
    n = len(x_values)
    result = table[0][0]  # Inisialisasi hasil dengan elemen tabel pertama

    for j in range(1, n):
        term = table[0][j]  # Ambil elemen dari tabel
        for i in range(j):
            term *= (x - x_values[i])  # Hitung suku Newton
        result += term  # Tambahkan ke hasil
